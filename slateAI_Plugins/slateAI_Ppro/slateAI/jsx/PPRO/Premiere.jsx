...
$._PPP_ = {
	...
	addClipMarker: function (mediaPath, time, rootItem, os) {
        if (os=="win32"){
            // replace "/" with "\" so Paramter "mediaPath" matches real mediaPath
            mediaPath = mediaPath.split("/").join("\\");            
        }

		//get project item to corresponding path
		var projectItemCounter = 0;
		while (projectItemCounter < rootItem.children.numItems) {
			var projectItem = rootItem.children[projectItemCounter];
			var projectItemPath = projectItem.getMediaPath();

			//alert("projectItemPath" + projectItemPath);
			//alert("projectItemPath" + mediaPath);

			//check if its a videoClip
			if (projectItem.type == ProjectItemType.CLIP || projectItem.type == ProjectItemType.FILE) {
				if (projectItemPath == mediaPath) {
					//correct ProjectItem found, 

					//CHECK IF MARKER TIME IS IN DURATION RANGE

					// load the XMP library to process metadata
					if (ExternalObject.AdobeXMPScript == undefined) {
						ExternalObject.AdobeXMPScript = new ExternalObject("lib:AdobeXMPScript");
					}
					//get metadata
					var xmpMeta = new XMPMeta(projectItem.getProjectMetadata());
					//check if duration is in metaData
					if (xmpMeta.doesPropertyExist("http://ns.adobe.com/premierePrivateProjectMetaData/1.0/", "premierePrivateProjectMetaData:Column.Intrinsic.MediaDuration")
						&& xmpMeta.doesPropertyExist("http://ns.adobe.com/premierePrivateProjectMetaData/1.0/", "premierePrivateProjectMetaData:Column.Intrinsic.MediaTimebase")) {
						//duration is in metadata and looks like: "00:00:05:03" = 0 hours, 0 minutes, 5 secs, 3 Frames
						var durationString = xmpMeta.getProperty("http://ns.adobe.com/premierePrivateProjectMetaData/1.0/", "premierePrivateProjectMetaData:Column.Intrinsic.MediaDuration").toString();
						//get seconds from durationString
						var durationSplits = durationString.split(":");
						var totalDurationSecs = 0;
						//hours
						totalDurationSecs += parseInt(durationSplits[0]) * 3600;
						//minutes
						totalDurationSecs += parseInt(durationSplits[1]) * 60;
						//seconds
						totalDurationSecs += parseInt(durationSplits[2]);
						//parts of a second
						var frameRateString = xmpMeta.getProperty("http://ns.adobe.com/premierePrivateProjectMetaData/1.0/", "premierePrivateProjectMetaData:Column.Intrinsic.MediaTimebase").toString();
						var frameRateFloat = parseFloat(frameRateString.replace(",", "."));
						totalDurationSecs += parseInt(durationSplits[3]) * (1 / frameRateFloat);
						//alert(totalDurationSecs.toString());
						//alert(time.toString())

						//compare marker time to duration: if marker time > duration: markert time = duration
						if (time > totalDurationSecs) {
							time = totalDurationSecs;
						}
					...
					if (markers) {
						...
						newMarker.name = 'SnycMarker';
						newMarker.comments = 'Use this Marker to sync Files.';
						//var tempTime = new Time();
						//tempTime.seconds = 15.6789;
						//newMarker.end = time + 3;
			...
			else if (projectItem.type == ProjectItemType.BIN || (projectItem.isSequence() === true)) {
				$._PPP_.addClipMarker(mediaPath, time, projectItem);
			}
			else {
				$._PPP_.updateEventPanel("warning: Trying to add Marker to Object which is not a Bin Clip or File");
			}
			projectItemCounter++;
		}
		if (rootItem.toString() == 'app.project.rootItem') {
			$._PPP_.updateEventPanel("Item to add clip to not found.");
		}

	},

	getSelectedItemPaths: function () {
					...
					if (currentProject.documentID === app.project.documentID) { // We're in the right project!
						...
						if (selectedItems) {
							for (var b = 0; b < selectedItems.length; b++) {
								...
								if (currentItem) {
									if (currentItem.type !== ProjectItemType.BIN && (currentItem.isSequence() === false)) { // For every selected item which is a video
										selectedItemPaths.push(currentItem.getMediaPath());
									}
									else { // For every selected item which is a bin or sequence...
										var binItemPaths = $._PPP_.getBinItemPathsRec(currentItem);
										for (var i = 0; i < binItemPaths.length; i++) {
											selectedItemPaths.push(binItemPaths[i]);
										}
									}
								} else {
	...
	getBinItemPathsRec: function (binObject) {
		var binItemPaths = [];
		for (var i = 0; i < binObject.children.numItems; i++) {
			var currentItem = binObject.children[i];
			if (currentItem.type !== ProjectItemType.BIN && (currentItem.isSequence() === false)) { // For every selected item which is a video
				binItemPaths.push(currentItem.getMediaPath());
				//alert("recursive: VIDEO"+ currentItem.getMediaPath());
			}
			else { // For every selected item which is a bin or sequence...
				//recursive call
				//alert("recursive: FOLDER");
				var nextBinItemPaths = $._PPP_.getBinItemPathsRec(currentItem);
				for (var k = 0; k < nextBinItemPaths.length; k++) {
					binItemPaths.push(nextBinItemPaths[k]);
				}
			}
	...
	getProjectItemPaths: function () {
		var projectItemPaths = [];
		var projectItemCounter = 0;
		while (projectItemCounter < app.project.rootItem.children.numItems) {
			projectItemPaths.push(app.project.rootItem.children[projectItemCounter].getMediaPath());
			projectItemCounter++;
		}
		//will be returned as string
		return projectItemPaths;
	},
	...
	keepPanelLoaded: function () {
		app.setExtensionPersistent("slateAI", 0); // 0, while testing (to enable rapid reload); 1 for "Never unload me, even when not visible."
	},
